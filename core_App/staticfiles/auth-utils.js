...
// Sincronizar localStorage con sessionStorage inmediatamente al cargar el script
try {
...
// Interceptar sessionStorage.setItem y sessionStorage.removeItem para mantener localStorage sincronizado
try {
    const originalSetItem = sessionStorage.setItem;
    sessionStorage.setItem = function(key, value) {
        originalSetItem.apply(this, arguments);
        if (key === 'jwt' || key === 'refresh_token') {
    ...
    console.error('Error interceptando sessionStorage:', e);
}
...
async function checkAuthOnPageLoad() {
    ...
    if (payload && payload.exp) {
        const now = Math.floor(Date.now() / 1000);
        if (now > payload.exp) {
            console.log('Token expirado al cargar la página, intentando renovar...');
            const refreshed = await AuthManager.refreshAccessToken();
            if (!refreshed) {
                console.log('No se pudo renovar el token al cargar la página, redirigiendo al login');
